...
export default function Home() {
 ...
 const playVideo = () => {
  ...
  if(videoRef.current){
    ...
    if(videoRef.current.paused === true || videoRef?.current.ended === false || videoRef?.current.currentTime < 1)
...
useEffect(() => {
  ...
  return (
    <div className=" min-h-screen flex flex-col items-center overflow-hidden">
      <main className="">
        <div className=''>
          <video width="full" height="full"   className='h-screen w-screen' ref={videoRef} muted style={{objectFit: 'cover'}} onWheel={()=> playVideo()} >
